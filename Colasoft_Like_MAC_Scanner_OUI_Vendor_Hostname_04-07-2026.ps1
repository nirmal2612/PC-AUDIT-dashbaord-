...
# 2) Optional external IEEE/OUI CSV beside this script: oui.csv / oui.txt / manuf.txt.
$VendorMap = @{
    '00:A8:59' = 'Fanvil'
    '0C:38:3E' = 'Fanvil'
    '00:09:6E' = 'Fanvil'
...
function Import-ExternalOuiMap {
    try {
        ...
        $files = @('oui.csv','oui.txt','manuf.txt') | ForEach-Object { Join-Path $base $_ } | Where-Object { Test-Path $_ }
        foreach ($file in $files) {
            foreach ($line in [System.IO.File]::ReadLines($file)) {
                ...
                $oui = ''
                $vendor = ''
                if ($l -match '^([0-9A-Fa-f]{2})[:-]([0-9A-Fa-f]{2})[:-]([0-9A-Fa-f]{2})[,\s\t]+(.+)$') {
                ...
                } elseif ($l -match '^([0-9A-Fa-f]{6})[,\s\t]+(.+)$') {
                    $x = $matches[1].ToUpper()
                    $oui = $x.Substring(0,2)+':'+$x.Substring(2,2)+':'+$x.Substring(4,2)
                    $vendor = $matches[2].Trim(' ',',','`t','"')
                }
                if ($oui -and $vendor -and -not $VendorMap.ContainsKey($oui)) { $VendorMap[$oui] = $vendor }
            }
        }
    } catch {}
}
...
function Get-DeviceType {
    ...
    if ($v -match 'zkteco|zksoftware|savior|access control|biometric') { return 'Access Control / Savior/ZKT' }
    if ($v -match 'fanvil|yealink|grandstream|polycom|avaya') { return 'IP Phone' }
    if ($v -match 'cisco|aruba|hp procurve|hpe|d-link|netgear|tp-link|mikrotik|ubiquiti|ruckus|extreme|brocade|h3c|huawei|juniper|arista') { return 'Switch / Network Device' }
    if ($v -match 'fortinet|palo alto|checkpoint|check point|sophos') { return 'Firewall / Security Device' }
    if ($v -match 'hikvision|dahua|camera|nvr|dvr') { return 'CCTV / Camera' }
    if ($v -match 'brother|canon|epson|lexmark|ricoh|printer') { return 'Printer' }
    if ($v -match 'dell|lenovo|hp|hewlett|acer|asus|msi|apple|microsoft|intel|vmware|virtualbox|hyper-v|kvm|xen') { return 'PC / Laptop / Server' }
    if (($Http -eq 'Open') -or ($Https -eq 'Open')) { return 'Web Device' }
...
function Resolve-HostNameSafe {
    ...
    # 2) NetBIOS fallback. Useful for Windows PCs where DNS reverse entry is missing.
    try {
...
$btnStart.Add_Click({
    ...
    for ($i = $from; $i -le $to; $i++) {
        ...
        if ($isOnline -or $mac) {
            ...
            $deviceType = Get-DeviceType -Vendor $vendor -HostName $host -Mac $mac -Http $http -Https $https
            $obj = [pscustomobject]@{
                SNo        = $sno
                IPAddress  = $ip
                MACAddress = $mac
            ...
            Add-ResultRow $obj
            Add-Log "Found $ip  MAC=$mac  OUI=$oui  Vendor=$vendor  Type=$deviceType  Host=$host  Status=$status"
        }
