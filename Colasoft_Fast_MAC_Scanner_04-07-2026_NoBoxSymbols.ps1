...
$title.Text = '[NET] Fast MAC Scanner - Colasoft Style'
...
$subtitle.Text = '[INFO] IP, MAC, Host Name, Workgroup, Manufacturer, Compare Result'
...
$btnOui.Add_Click({
    $lblStatus.Text = 'Status: Downloading official IEEE OUI database...'
    [System.Windows.Forms.Application]::DoEvents()
...
$btnStop.Add_Click({
    ...
    $lblStatus.Text = 'Status: Stopping scan...'
})
...
$btnScan.Add_Click({
        ...
        Start-FastScan -IPs $ips -TimeoutMs ([int]$numTimeout.Value) -Threads ([int]$numThreads.Value) `
            ...
            -OnProgress {
                ...
                if ($total -gt 0) {
                    ...
                    $progress.Value = $pct
                    $lblStatus.Text = "Status: Progress $pct%"
                }
            }
    } catch {
    ...
    $progress.Value = 100
    $lblStatus.Text = "Status: Completed in $([Math]::Round($swTotal.Elapsed.TotalSeconds,1)) sec"
