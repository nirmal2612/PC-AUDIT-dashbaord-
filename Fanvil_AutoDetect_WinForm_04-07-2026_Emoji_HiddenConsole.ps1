...
function Stop-Scan {
    ...
    Write-AppLog '⏹️ Stop requested. Finishing current IP check...'
...
function Start-Scan {
    ...
    Write-AppLog "▶️ Scan started for $segment.$start to $segment.$end"

    for ($i=$start; $i -le $end; $i++) {
        if ($script:stopScanRequested) { Write-AppLog '⏹️ Scan stopped by user.'; break }
        $ip = "$segment.$i"
        $count++
        ...
        $brand = if ($isFanvilOui) { '☎️ Fanvil' } else { '🌐 Web Device' }
        $model = ''
        $extension = ''
        $sipUser = ''
        ...
        if ($isFanvilOui -and $user -and $pass) {
            Write-AppLog "☎️ Fanvil OUI matched: $ip - trying login/read"
    ...
    if ($results.Count -gt 0) {
        ...
        Write-AppLog 'ℹ️ No web/Fanvil devices found.'
        [System.Windows.Forms.MessageBox]::Show('ℹ️ No web/Fanvil devices found.','Completed','OK','Information') | Out-Null
    }
...
$note.Text = '📂 Output: D:\fanvil | ☎️ Fanvil only if MAC OUI matches: 00:A8:59, 0C:38:3E, 00:09:6E | 🌐 URL saved for every web IP.'
