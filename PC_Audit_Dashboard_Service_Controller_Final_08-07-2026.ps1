...
th { user-select:none; }
.filter-btn { margin-left:7px; border:1px solid #667; background:#222831; color:#fff; border-radius:4px; padding:2px 5px; cursor:pointer; font-size:11px; }
.filter-btn.active { background:#f39c12; color:#111; }
.filter-menu { position:fixed; z-index:99999; width:260px; background:#222831; border:1px solid #555; border-radius:8px; box-shadow:0 10px 24px rgba(0,0,0,.45); padding:10px; }
.filter-menu input { width:100%; padding:8px; background:#11161d; color:#fff; border:1px solid #555; border-radius:6px; margin-bottom:8px; }
.filter-actions { display:flex; gap:8px; }
.filter-actions button { flex:1; padding:7px; border:none; border-radius:5px; cursor:pointer; font-weight:600; }
.apply-filter { background:#2ecc71; color:#fff; }
.clear-filter { background:#95a5a6; color:#111; }
...
function applyTableFilters(){
    document.querySelectorAll('#t tbody tr').forEach(tr => {
        ...
        Object.keys(columnFilters).forEach(k => {
            ...
            if (val && !normalizeText(cells[parseInt(k)] ? cells[parseInt(k)].innerText : '').includes(normalizeText(val))) { colOk = false; }
        });
        tr.style.display = colOk ? '' : 'none';
    });
}
function closeFilterMenu(){ const old = document.getElementById('filterMenu'); if(old){ old.remove(); } }
function openColumnFilter(btn, colIndex){
    closeFilterMenu();
    const menu = document.createElement('div');
    menu.id = 'filterMenu';
    menu.className = 'filter-menu';
    menu.innerHTML = '<input id="filterInput" placeholder="Search this column..." value="'+(columnFilters[colIndex] || '').replace(/"/g,'&quot;')+'"><div class="filter-actions"><button class="apply-filter">Apply</button><button class="clear-filter">Clear</button></div>';
    document.body.appendChild(menu);
    const r = btn.getBoundingClientRect();
    menu.style.left = Math.min(r.left, window.innerWidth - 275) + 'px';
    menu.style.top = (r.bottom + 6) + 'px';
    const inp = menu.querySelector('#filterInput'); inp.focus(); inp.select();
    menu.querySelector('.apply-filter').onclick = function(){ columnFilters[colIndex] = inp.value; btn.classList.toggle('active', !!inp.value); closeFilterMenu(); applyTableFilters(); };
    menu.querySelector('.clear-filter').onclick = function(){ delete columnFilters[colIndex]; btn.classList.remove('active'); closeFilterMenu(); applyTableFilters(); };
    inp.addEventListener('keydown', function(e){ if(e.key === 'Enter'){ menu.querySelector('.apply-filter').click(); } if(e.key === 'Escape'){ closeFilterMenu(); } });
}
function setupExcelFilters(){
    document.querySelectorAll('#t thead th').forEach((th, idx) => {
        const b = document.createElement('button');
        b.className = 'filter-btn';
        b.title = 'Filter / search this column';
        b.textContent = '▼';
        b.onclick = function(e){ e.stopPropagation(); openColumnFilter(b, idx); };
        th.appendChild(b);
    });
    document.addEventListener('click', function(e){ const m=document.getElementById('filterMenu'); if(m && !m.contains(e.target) && !e.target.classList.contains('filter-btn')) closeFilterMenu(); });
}
setupExcelFilters();
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebCredential -Context $context)) {
...
# ====================================================================================================
# SECURITY SETTINGS - WEB BASIC LOGIN + CONTROLLER PASSWORD SUPPORT
# ====================================================================================================
function Protect-PlainTextPassword {
    param([string]$Text)
    if ($null -eq $Text) { $Text = "" }
    try { return ($Text | ConvertTo-SecureString -AsPlainText -Force | ConvertFrom-SecureString) } catch { return $Text }
}

function Unprotect-PlainTextPassword {
    param([string]$ProtectedText, [string]$Fallback)
    if ([string]::IsNullOrWhiteSpace($ProtectedText)) { return $Fallback }
    try {
        $sec = $ProtectedText | ConvertTo-SecureString
...
function Get-GuiSecret {
    param([string]$Name, [string]$Fallback)
    try {
        $v = $script:GuiSettings.$Name
        return (Unprotect-PlainTextPassword -ProtectedText ([string]$v) -Fallback $Fallback)
    } catch { return $Fallback }
}

function Set-GuiSecret {
    param([string]$Name, [string]$Value)
    $script:GuiSettings.$Name = Protect-PlainTextPassword $Value
}

function Test-ControllerCredential {
    param([string]$UserName, [string]$Password)
    $u = Get-GuiSecret -Name 'ControllerUser' -Fallback 'HH0010520'
    $p = Get-GuiSecret -Name 'ControllerPassword' -Fallback 'Foxconn-FXCN-IT'
    return (($UserName.Trim() -ieq $u) -and ($Password -eq $p))
}

function Test-WebCredential {
    param([System.Net.HttpListenerContext]$Context)
    $expectedUser = Get-GuiSecret -Name 'WebUser' -Fallback 'PCAudit'
    $expectedPass = Get-GuiSecret -Name 'WebPassword' -Fallback 'PC_Audit'
    $auth = $Context.Request.Headers['Authorization']
    if ([string]::IsNullOrWhiteSpace($auth) -or -not $auth.StartsWith('Basic ')) { return $false }
    try {
        $raw = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($auth.Substring(6).Trim()))
        $idx = $raw.IndexOf(':')
        if ($idx -lt 0) { return $false }
        $u = $raw.Substring(0,$idx)
        $p = $raw.Substring($idx+1)
        return (($u -ieq $expectedUser) -and ($p -eq $expectedPass))
    } catch { return $false }
}
...
function Show-CredentialChangeWindow {
    param(
        [string]$Mode,
        [string]$UserProperty,
    ...
    $cf.Text = "Change $Mode Credential"
    $cf.Size = New-Object System.Drawing.Size(430, 310)
    $cf.StartPosition = "CenterParent"
    $cf.FormBorderStyle = "FixedDialog"
    $cf.MaximizeBox = $false
    $cf.MinimizeBox = $false
    $cf.BackColor = [System.Drawing.Color]::FromArgb(27,31,36)
    $cf.ForeColor = [System.Drawing.Color]::White

    function Add-CredLabel($text,$y) {
        $l = New-Object System.Windows.Forms.Label
        $l.Text=$text; $l.Location=New-Object System.Drawing.Point(28,$y); $l.Size=New-Object System.Drawing.Size(145,22)
        $l.ForeColor=[System.Drawing.Color]::Gainsboro
        $cf.Controls.Add($l)
    }
    function Add-CredText($text,$y,$password=$false) {
        $t=New-Object System.Windows.Forms.TextBox
        $t.Text=$text; $t.Location=New-Object System.Drawing.Point(180,$y); $t.Size=New-Object System.Drawing.Size(200,24)
        ...
        $cf.Controls.Add($t); return $t
    }

    Add-CredLabel "Current Username" 32
    $txtUser = Add-CredText (Get-GuiSecret -Name $UserProperty -Fallback $DefaultUser) 30 $false
    Add-CredLabel "Old Password" 72
    $txtOld = Add-CredText "" 70 $true
    Add-CredLabel "New Password" 112
    $txtNew = Add-CredText "" 110 $true
    Add-CredLabel "Confirm Password" 152
    $txtConfirm = Add-CredText "" 150 $true

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(180,205); $btnSave.Size=New-Object System.Drawing.Size(90,32)
    ...
    $cf.Controls.Add($btnSave)
    $btnCancel = New-Object System.Windows.Forms.Button
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(290,205); $btnCancel.Size=New-Object System.Drawing.Size(90,32)
    ...
    $cf.Controls.Add($btnCancel)
    $btnCancel.Add_Click({ $cf.Close() })
    $btnSave.Add_Click({
        $currentPass = Get-GuiSecret -Name $PasswordProperty -Fallback $DefaultPassword
        if ($txtOld.Text -ne $currentPass) {
            [System.Windows.Forms.MessageBox]::Show("Old password is wrong.", "Change Password", "OK", "Warning") | Out-Null
            $txtOld.Clear(); $txtOld.Focus(); return
        }
        if ([string]::IsNullOrWhiteSpace($txtUser.Text.Trim())) {
            [System.Windows.Forms.MessageBox]::Show("Username cannot be empty.", "Change Password", "OK", "Warning") | Out-Null; return
        }
        if ([string]::IsNullOrWhiteSpace($txtNew.Text) -or $txtNew.Text.Length -lt 4) {
            [System.Windows.Forms.MessageBox]::Show("New password must be at least 4 characters.", "Change Password", "OK", "Warning") | Out-Null; return
        }
        if ($txtNew.Text -ne $txtConfirm.Text) {
            [System.Windows.Forms.MessageBox]::Show("New password and confirm password do not match.", "Change Password", "OK", "Warning") | Out-Null; return
        }
        $script:GuiSettings.$UserProperty = $txtUser.Text.Trim()
        Set-GuiSecret -Name $PasswordProperty -Value $txtNew.Text
        Save-GuiSettings
        [System.Windows.Forms.MessageBox]::Show("$Mode credential changed successfully.", "Change Password", "OK", "Information") | Out-Null
        Add-GuiLog "$Mode credential changed from Settings." "INFO"
        $cf.Close()
    })
    $cf.ShowDialog($script:MainForm) | Out-Null
}

function Invoke-ControllerLock {
    if ($script:ControllerLockActive) { return }
    $script:ControllerLockActive = $true
    try {
        Add-GuiLog "Controller auto locked due to inactivity." "WARN"
        while ($true) {
            if (Show-LoginWindow -LockMode) { break }
        }
        Add-GuiLog "Controller unlocked." "INFO"
    } finally {
        $script:ControllerLockActive = $false
        $script:LastControllerActivity = Get-Date
    }
}

# GUI globals
$script:GuiVersion = "2026.07.08 Login AutoLock Filter Update"
...
function Initialize-GuiSettings {
    $defaults = [ordered]@{
        ...
        AutoRefreshLog = $true
        WebUser = "PCAudit"
        WebPassword = (Protect-PlainTextPassword "PC_Audit")
        ControllerUser = "HH0010520"
        ControllerPassword = (Protect-PlainTextPassword "Foxconn-FXCN-IT")
        ControllerAutoLockMinutes = 5
    }
...
function Show-SettingsWindow {
    ...
    $f.Size = New-Object System.Drawing.Size(660, 545)
    ...
    New-Label "Auto Lock Minutes" 24 324 | Out-Null
    $txtLock = New-Text ([string]$script:GuiSettings.ControllerAutoLockMinutes) 190 322 110

    $btnWebCred = New-Object System.Windows.Forms.Button
    $btnWebCred.Text="CHANGE WEB CREDENTIAL"; $btnWebCred.Location=New-Object System.Drawing.Point(190,354); $btnWebCred.Size=New-Object System.Drawing.Size(190,32)
    $btnWebCred.BackColor=[System.Drawing.Color]::FromArgb(52,152,219); $btnWebCred.ForeColor=[System.Drawing.Color]::White; $btnWebCred.FlatStyle='Flat'
    $f.Controls.Add($btnWebCred)

    $btnControllerCred = New-Object System.Windows.Forms.Button
    $btnControllerCred.Text="CHANGE CONTROLLER LOGIN"; $btnControllerCred.Location=New-Object System.Drawing.Point(394,354); $btnControllerCred.Size=New-Object System.Drawing.Size(190,32)
    $btnControllerCred.BackColor=[System.Drawing.Color]::FromArgb(155,89,182); $btnControllerCred.ForeColor=[System.Drawing.Color]::White; $btnControllerCred.FlatStyle='Flat'
    $f.Controls.Add($btnControllerCred)

    $btnWebCred.Add_Click({ Show-CredentialChangeWindow -Mode "Web" -UserProperty "WebUser" -PasswordProperty "WebPassword" -DefaultUser "PCAudit" -DefaultPassword "PC_Audit" })
    $btnControllerCred.Add_Click({ Show-CredentialChangeWindow -Mode "Controller" -UserProperty "ControllerUser" -PasswordProperty "ControllerPassword" -DefaultUser "HH0010520" -DefaultPassword "Foxconn-FXCN-IT" })

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(374,448); $btnSave.Size=New-Object System.Drawing.Size(95,34)
    $btnSave.BackColor=[System.Drawing.Color]::FromArgb(46,204,113); $btnSave.ForeColor=[System.Drawing.Color]::White; $btnSave.FlatStyle='Flat'
    $f.Controls.Add($btnSave)
    $btnCancel = New-Object System.Windows.Forms.Button
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(486,448); $btnCancel.Size=New-Object System.Drawing.Size(95,34)
    $btnCancel.BackColor=[System.Drawing.Color]::FromArgb(95,101,112); $btnCancel.ForeColor=[System.Drawing.Color]::White; $btnCancel.FlatStyle='Flat'
    ...
    $btnSave.Add_Click({
        ...
        $lockMinutes = 5
        if (-not [int]::TryParse($txtLock.Text.Trim(), [ref]$lockMinutes) -or $lockMinutes -lt 1) { $lockMinutes = 5 }
        $script:GuiSettings.ControllerAutoLockMinutes = $lockMinutes
        Save-GuiSettings
        Apply-GuiSettings
...
function Show-LoginWindow {
    param([switch]$LockMode)
    $login = New-Object System.Windows.Forms.Form
    $login.Text = if ($LockMode) { "🔒 Controller Locked" } else { "🔑 PC Audit Login" }
    ...
    $txtUser.Location=New-Object System.Drawing.Point(150,86); $txtUser.Size=New-Object System.Drawing.Size(200,25); $txtUser.Text=(Get-GuiSecret -Name 'ControllerUser' -Fallback 'HH0010520')
    ...
    $btnExit.Add_Click({ if ($LockMode) { return } else { $login.Close() } })
    if ($LockMode) { $btnExit.Enabled = $false; $login.ControlBox = $false }
    $btnLogin.Add_Click({
        if (Test-ControllerCredential -UserName $txtUser.Text -Password $txtPass.Text) {
            $script:LoginOk = $true
            $login.Close()
        } else {
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    # 5-minute inactivity auto-lock. Timer checks user activity on controller window.
    $script:LastControllerActivity = Get-Date
    $script:ControllerLockActive = $false
    $activityHandler = { $script:LastControllerActivity = Get-Date }
    $form.Add_MouseMove($activityHandler)
    $form.Add_MouseDown($activityHandler)
    $form.Add_KeyDown($activityHandler)
    $form.KeyPreview = $true
    foreach ($ctrl in $form.Controls) {
        try { $ctrl.Add_MouseMove($activityHandler); $ctrl.Add_MouseDown($activityHandler); $ctrl.Add_KeyDown($activityHandler) } catch { }
    }
    $lockTimer = New-Object System.Windows.Forms.Timer
    $lockTimer.Interval = 30000
    $lockTimer.Add_Tick({
        try {
            $mins = [int]$script:GuiSettings.ControllerAutoLockMinutes
            if ($mins -lt 1) { $mins = 5 }
            if (-not $script:ControllerLockActive -and ((New-TimeSpan -Start $script:LastControllerActivity -End (Get-Date)).TotalMinutes -ge $mins)) {
                Invoke-ControllerLock
            }
        } catch { }
    })
    $lockTimer.Start()
    $form.Add_FormClosed({ try { $lockTimer.Stop(); $lockTimer.Dispose() } catch { } })
